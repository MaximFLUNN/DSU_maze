...
public:
	...
	void unite_without_an_enemy(std::vector<std::pair<int, int>> enemies) {
		for (int i = 0; i < m_size; i++) {
			std::unordered_map<int, int> wrong;
			for (auto e : enemies) {
				if (e.first == i) {
					auto search = wrong.find(e.second);
					if (search == wrong.end()) {
						wrong.insert({ e.second, find(e.second) });
					}
				}
				else if (e.second == i) {
					auto search = wrong.find(e.first);
					if (search == wrong.end()) {
						wrong.insert({ e.first, find(e.first) });
					}
				}
			}
			for (int j = 0; j < i; j++) {
...
int main() {
	...
	dsu2.unite_without_an_enemy(enemies);

	dsu2.print();
	std::cout << " <- Step: unite without an enemy" << std::endl;
